/*
 * @Author: Weidows
 * @Date: 2020-07-09 21:10:53
 * @LastEditors: Weidows
 * @LastEditTime: 2020-11-22 16:26:33
 * @FilePath: \Weidows\C++\Project\贪吃蛇\18行.c
 */
//头文件
#include <conio.h>
#include <windows.h>
int main()
{
    ...
    for (map[rand() % 900] = -1; 1; Sleep(100))
    {
        if (_kbhit() && (cl = _getch()) &&cl < 97 ? cl += 32 : 1)
            if (cl == 'a' && c != 'd' || cl == 'd' && c != 'a' || cl == 'w' && c != 's' || cl == 's' && c != 'w')
                c = cl;
        if (c == 'a' && --hY < 0 || c == 'd' && ++hX == 30 || c == 'w' && --hY < 0 || c == 's' && ++hY == 30)
            exit(0);
        if (map[hY * 30 + hX] && (map[hY * 30 + hX] > 0 ? exit(0), 1 : ++len))
            for (i = rand() % 900; map[i] ||
                                   !(map[i] = -1);
                 i = rand() % 900)
                ;
        else
